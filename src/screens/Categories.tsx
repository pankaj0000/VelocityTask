...
type TWorkerData = {
  ...
  worker_country_flag_image: string;
  worker_profile_picture: string;
};

let searchExpirationTimeout: NodeJS.Timeout | undefined = undefined;

export default function Categories() {
  const [selectedCategory, setSelectedCategory] = useState(1);
  const [searchValue, setSearchValue] = useState('');
  const [workerData, setWorkerData] = useState<TWorkerData[]>([]);
  ...
  function prepareWorkerData(categoryId?: number, searchValue?: string) {
  ...
  function handleChangeCategory(selectedCategoryId: number) {
  ...
  const renderItem = ({item}: {item: TWorkerData}) => {
    return (
  ...
  return (
    <View style={styles.container}>
      ...
      <View
        ...
        <IconButton
          containerStyle={{marginLeft: Dimension.Size_15}}
